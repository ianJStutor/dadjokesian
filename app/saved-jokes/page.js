...
export default async function SavedJokes() {
    ...
    if (userData.user) {
        const { data: jokesData, error } = await supabase
            .from("joke")
            .select("*");
        if (error) console.error("Error fetching jokes", error);
        else jokes = jokesData;
    }

    let header = "Saved Jokes";
    if (!jokes.length) header = "Save some jokes!";
    if (!userData.user) header = "Login to save your favorite jokes";
